...
### Fixed
- `? table` could take several seconds to minutes, or appear to hang, on a schema with a wide "hub" table — e.g. a shared tenant/account table referenced directly by hundreds of others, a common multi-tenant shape. Confirmed by timing a synthetic 150-table fixture: 4.3 seconds before this fix, 14ms after. Two compounding causes: the transitive-redundancy check recomputed the same reachability search from scratch for every candidate route instead of reusing it, and the search re-sorted its *entire* remaining set of candidate routes on every single step instead of using a proper priority queue. Both fixed; ordering is unaffected (all existing results and their order are unchanged).
