...
## [Unreleased]
### Added
- MySQL 8.0+ as a second, fully-parallel database backend alongside Postgres — identifier quoting, date bucketing, casts, and update!/delete! all render MySQL-correct SQL, and schema introspection is scoped to the connected database via `DATABASE()`. MySQL 5.7 isn't supported: Pine emits `WITH` (CTEs) for `count:`, `group:`, every `|=` variable, and auto-checkpoints, and 5.7 has no CTE support. Covered by the same fixture-based test suite Postgres already has (no live database in CI), plus manual verification against a real MySQL server (`dev.docker-compose.yml`) and through beamlynx-ui's new connection-type picker. Desktop packaging (`modules.list`) needed no changes — confirmed by building and running a real jpackage app-image against it.
### Fixed
- A query against a MySQL connection touching any `DATETIME`/`DATE`/`TIME` column failed with a silent 500 and no server-side log: MySQL's JDBC driver returns `java.time.LocalDateTime`/`LocalDate`/`LocalTime` values, which Cheshire (the JSON library) had no encoder for, and the resulting `JsonGenerationException` was thrown while writing the HTTP response — after `wrap-cors`, so the response reached the browser with no CORS header at all, which is why it showed up there as a confusing "blocked by CORS policy" error rather than a 500.
### Changed
