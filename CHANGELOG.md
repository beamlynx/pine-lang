...
### Fixed
- `? table` no longer suggests paths that route back through a table the expression has already joined. The search only ever excluded the *last* table in the pipe, so `company | employee | document | ? company` came back with three routes, every one of them re-entering a table already in the query — and `company | employee | ? document` included a detour back up through `company`. Those suggestions did evaluate (Pine auto-aliases the second occurrence), they were just never answers to the question being asked: `?` looks for a route the expression doesn't already have. A target now reachable only through already-joined tables correctly returns nothing. Two things are deliberately still joinable: a table sealed into a CTE by a checkpoint (`l:`/`group:`), since the outer query no longer joins it, and the real table behind a variable. The table-name suggestions served while the target is still being typed apply the same exclusion, so they can't offer a name that would resolve to zero paths.
- Comma-separated conditions inside one `where:` segment now combine with `OR`, e.g. `where: id = 1, id = 2` produces `WHERE (id = ? OR id = ?)`. The grammar already accepted this syntax, but the parser kept only the first condition and silently dropped the rest, so a comma inside `where:` was effectively a no-op past the first condition. `AND` is unaffected and still works the same way, by chaining separate `where:` steps (`where: id = 1 | name = 'Acme'`).
